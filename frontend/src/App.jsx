      ...
      <div className="responsive-navbar-inner responsive-padding" style={{
        ...
        <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: '10px', textDecoration: 'none', marginRight: '40px', flexShrink: 0 }}>
          <div style={{ width: '34px', height: '34px', borderRadius: '10px', background: 'linear-gradient(135deg, #e50914 0%, #ff4040 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 16px rgba(229,9,20,0.45)', border: '1px solid rgba(255,100,100,0.2)' }}>
            <span style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, color: 'var(--text-main)', fontSize: '16px', lineHeight: 1 }}>W</span>
          </div>
          <span style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, color: 'var(--text-main)', fontSize: '18px', letterSpacing: '0.08em', textTransform: 'uppercase' }}>WAITLESS</span>
        </Link>

        {/* Location Selector */}
        <div style={{ position: 'relative', marginRight: '24px' }}>
          <button onClick={() => setShowCities(!showCities)} style={{ display:'flex', alignItems:'center', gap:'6px', background: showCities ? 'rgba(229,9,20,0.1)' : 'rgba(var(--rgb-white),0.04)', border:'1px solid ' + (showCities ? 'rgba(229,9,20,0.25)' : 'rgba(var(--rgb-white),0.07)'), color:'#e2e8f0', cursor:'pointer', fontSize:'13px', fontWeight:600, padding:'6px 14px', borderRadius:'99px', transition: 'all 0.2s ease' }}>
            📍 <span style={{ color:'#f87171' }}>{city}</span> <span style={{ color: '#4b5563' }}>▾</span>
          </button>
          {showCities && (
            <div style={{ position:'absolute', top:'calc(100% + 8px)', left:0, background:'rgba(14,16,26,0.95)', backdropFilter: 'blur(20px)', border:'1px solid rgba(var(--rgb-white),0.08)', borderRadius:'14px', padding:'8px', zIndex:50, minWidth:'160px', boxShadow:'0 16px 40px rgba(0,0,0,0.6)' }}>
              {CITIES.map(c => (
                <button key={c} onClick={() => { changeCity(c); setShowCities(false) }} style={{ display:'block', width:'100%', textAlign:'left', padding:'9px 14px', background: city===c?'rgba(229,9,20,0.12)':'transparent', color: city===c?'#f87171':'var(--text-muted)', border:'none', borderRadius:'8px', cursor:'pointer', fontSize:'13px', fontWeight: city===c ? 700 : 400, transition: 'all 0.15s' }}>
                  {c}
                </button>
              ))}
            </div>
          ...
          {NAV_LINKS.map(({ to, label }) => {
            ...
            return (
              <Link key={to} to={to} style={{
                ...
                color: isActive ? 'var(--text-main)' : '#4b5563',
                background: isActive ? 'rgba(var(--rgb-white),0.09)' : 'transparent',
                border: isActive ? '1px solid rgba(var(--rgb-white),0.1)' : '1px solid transparent',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          {/* Theme Toggle */}
          <button onClick={toggleTheme} style={{ display:'flex', alignItems:'center', justifyContent:'center', width:'34px', height:'34px', borderRadius:'50%', background:'rgba(var(--rgb-white),0.05)', border:'1px solid rgba(var(--rgb-white),0.1)', color:'var(--text-main)', cursor:'pointer', transition:'all 0.2s', marginRight:'4px' }}>
            {theme === 'dark' ? '☀️' : '🌙'}
          </button>
          ...
          {user ? (
            <>
              ...
              {user.role === 'provider' && (
                <Link to="/provider" style={{ color: 'var(--text-muted)', fontSize: '13px', fontWeight: 600, textDecoration: 'none' }}>Dashboard</Link>
              )}
              <Link to="/profile" style={{ display: 'flex', alignItems: 'center', textDecoration: 'none' }}>
                <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'linear-gradient(135deg, #e50914, #f97316)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-main)', fontSize: '13px', fontWeight: 800, boxShadow: '0 0 0 2px rgba(229,9,20,0.3)' }}>
                  {user.name.charAt(0).toUpperCase()}
                </div>
              </Link>
              <button onClick={handleLogout} style={{ padding: '7px 16px', fontSize: '13px', fontWeight: 600, borderRadius: '99px', background: 'rgba(var(--rgb-white),0.04)', border: '1px solid rgba(var(--rgb-white),0.07)', color: '#4b5563', cursor: 'pointer', transition: 'all 0.2s' }}>
                Logout
              </button>
            </>
          ) : (
            <>
              <Link to="/login" style={{ padding: '7px 18px', fontSize: '13px', fontWeight: 600, textDecoration: 'none', borderRadius: '99px', color: 'var(--text-muted)', border: '1px solid rgba(var(--rgb-white),0.08)', background: 'transparent', transition: 'all 0.2s' }}>
                Sign In
              </Link>
        ...
        <AuthProvider>
          <LocationProvider>
            <ToastProvider>
              <BrowserRouter>
                <NavBar />
                <Suspense fallback={<div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'var(--bg-main)', color:'var(--text-main)' }}>Loading...</div>}>
                <Routes>
