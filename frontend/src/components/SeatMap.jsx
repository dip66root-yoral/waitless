...
export function SeatMap({ numTickets = 2, accent = '#e50914', selectedSeats, onSelect }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '540px', background: 'var(--bg-main)', borderRadius: '16px', overflow: 'hidden', border: '1px solid rgba(var(--rgb-white),0.05)' }}>
      {/* Zoom Controls */}
      <div style={{ position: 'absolute', top: 16, right: 16, display: 'flex', flexDirection: 'column', gap: 8, zIndex: 10 }}>
        <button onClick={() => setScale(s => Math.min(2, s + 0.2))} style={{ width: 32, height: 32, borderRadius: '8px', background: 'rgba(var(--rgb-white),0.1)', border: '1px solid rgba(var(--rgb-white),0.1)', color: 'var(--text-main)', fontSize: '18px', cursor: 'pointer', backdropFilter: 'blur(10px)' }}>+</button>
        <button onClick={() => setScale(s => Math.max(0.5, s - 0.2))} style={{ width: 32, height: 32, borderRadius: '8px', background: 'rgba(var(--rgb-white),0.1)', border: '1px solid rgba(var(--rgb-white),0.1)', color: 'var(--text-main)', fontSize: '18px', cursor: 'pointer', backdropFilter: 'blur(10px)' }}>-</button>
      </div>
        ...
        <div style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${scale})`, transition: isDragging ? 'none' : 'transform 0.1s', display: 'flex', flexDirection: 'column', gap: '30px', padding: '40px' }}>
          ...
          {SECTIONS.map(section => (
            <div key={section.id} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              {/* Section Header */}
              <div style={{ borderBottom: '1px solid rgba(var(--rgb-white),0.1)', paddingBottom: '4px', marginBottom: '8px', display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontSize: '12px', color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em' }}>{section.label}</span>
                <span style={{ fontSize: '12px', color: 'var(--text-muted)', fontWeight: 600 }}>₹{section.price}</span>
              </div>
                ...
                <div key={row} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '6px' }}>
                    {[...Array(20)].map((_, i) => {
                      ...
                      let bg = 'rgba(var(--rgb-white),0.08)'
                      let border = '1px solid rgba(var(--rgb-white),0.15)'
                      if (isBooked) {
                        bg = 'rgba(var(--rgb-white),0.03)'
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            color: isSelected ? 'var(--text-main)' : 'transparent',
          ...
          <div style={{ marginTop: '40px', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            <p style={{ color: 'var(--text-main)', fontSize: '12px', fontWeight: 700, letterSpacing: '0.3em', textTransform: 'uppercase', margin: '-30px 0 0', opacity: 0.8, textShadow: '0 2px 10px rgba(0,0,0,1)' }}>All Eyes This Way</p>
          </div>
